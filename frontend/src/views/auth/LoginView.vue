<template>
  <div class="auth-wrap page-enter">
    <el-card class="soft-card auth-card">
      <h2 class="section-title">欢迎登录</h2>
...
const router = useRouter(); const store = useUserStore(); const formRef = ref()
...
const submit = async () => { await formRef.value.validate(); await store.login(form.value); ElMessage.success('登录成功'); router.push('/') }
</script>
<style scoped>
